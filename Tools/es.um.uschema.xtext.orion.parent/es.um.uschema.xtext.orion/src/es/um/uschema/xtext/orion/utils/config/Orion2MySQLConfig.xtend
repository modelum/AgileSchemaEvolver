...
class Orion2MySQLConfig {
	...
	new() {
		...
		this.inputPath = null
		this.outputPath = Path.of("../es.um.uschema.xtext.athena/models/orion/output/sql/")
	}
	...
	new(String[] args) {
		...
		options.addOption(
			Option.builder("o").longOpt("output").desc("Path to an output folder").hasArg().argName("output_folder").
				build())

		val inputMode = new OptionGroup();
		inputMode.setRequired(true);
		inputMode.addOption(
			Option.builder("i").longOpt("input").desc("Path to an input model").hasArg().argName("model").build())
		inputMode.addOption(
			Option.builder("f").longOpt("folder").desc("Path to an input folder").hasArg().argName("input_folder").
				build())
		...
		try {
			...
			if (cmd.hasOption("h")) {
				formatter.printHelp("<Orion2SQL Main>", options)
				System.exit(0)
			}
